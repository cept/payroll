<x-layout>
    ...
    <a href="{{ route('bagian.create') }}" class="middle none center mr-4 rounded-lg bg-blue-500 py-3 px-6 font-sans text-xs font-bold uppercase text-white shadow-md shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none" data-ripple-light="true">
    Tambah Bagian
    </a>
    ...
    <table class="table-fixed w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr
                ...
                <td class="px-6 py-4">
                    <form onsubmit="return confirm('Apakah anda yakin ? data karyawan yang berkaitan juga akan terhapus!');" action="{{ route('bagian.destroy', $bagian->id) }}" method="post">
                        <a href="{{ route('bagian.show', $bagian->id) }}" class="font-medium text-green-600 dark:text-green-500 hover:underline mr-2">Show</a>
                        <a href="{{ route('bagian.edit', $bagian->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-2">Edit</a>
                    @csrf
                    @method('DELETE')
