<x-layout>
    ...
    <a href="{{ route('penggajian.create') }}" class="middle none center mr-4 rounded-lg bg-blue-500 py-3 px-6 font-sans text-xs font-bold uppercase text-white shadow-md shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none" data-ripple-light="true">
        Tambah Penggajian
        </a>
    
        <div class="flex gap-4 mt-6 mb-3">
            <form action="{{ route('penggajian.search') }}" method="post">
            @csrf
                ...
                <tr
                    ...
                    <td class="px-6 py-4">
                        <form onsubmit="return confirm('Apakah anda yakin ?');" action="{{ route('penggajian.destroy', $penggajian->id) }}" method="post">
                            <a href="{{ route('penggajian.show', $penggajian->id) }}" class="font-medium text-green-600 dark:text-green-500 hover:underline mr-2">Show</a>
                            <a href="{{ route('penggajian.edit', $penggajian->id) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-2">Edit</a>
                        @csrf
                        @method('DELETE')
